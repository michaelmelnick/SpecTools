...
def _TFTLinearTransfer(Vgs, u, Vt, Vds, W, L, Cox):
    """
    This is the linear-range transconductance equation for modelling the source-drain
    current in a thin film transistor from input voltages and device geometry.

    Vgs - an array of Gate-Source voltages
    u - mobility
    Vt - threshold voltage

    These should not be refined in the fit
    Vds - the drain-source volatage 
    W - channel width
    L - channel length
    Cox - Oxide capacitance
    """
    return W*u*Vds*Cox*(Vgs - Vt - (Vds/2.))/L


def _TFTLinearOutput(Vgs, u, Vt, Vds, W, L, Cox):
    """
    This is the linear-range transconductance equation for modelling the source-drain
    current in a thin film transistor from input voltages and device geometry.

    Vds - an array of drain-source volatages
    u - mobility
    Vt - threshold voltage
...
def _TFTSaturationTransfer(Vgs, u, Vt, W, L, Cox):
    """
    The is the saturation range transconance equation for modelling source-drain
    current in a thin film transistor from input voltages and device geometry.
...
ptypes = {  'gauss': ['Amplitude', 'Width', 'Position'],
            ...
            'TFTLinearTransfer':['Mobility','Threshold Voltage', 'V$_{DS}$', 'Width', 'Length', 'Cox'],
            'TFTLinearOutput':['Mobility','Threshold Voltage', 'V$_{GS}$', 'Width', 'Length', 'Cox'],
            'TFTSaturationTransfer':['Mobility','Threshold Voltage', 'Width', 'Length', 'Cox']
...
funcs = {
            ...
            'TFTLinearTransfer':_TFTLinearTransfer,
            'TFTLinearOutput':_TFTLinearOutput,
            'TFTSaturationTransfer':_TFTSaturationTransfer       
        }
...
class Feature:
    ...
    ptypes = 
    Known ptypes and parameters:
    
    'gauss'    : ['Amplitude', 'Width', 'Position'],
    'lorentz'  : ['Amplitude', 'Width', 'Position'],
    'scatter'  : ['A','B'],
    'flat'     : ['Baseline'],
    'line'     : ['Slope','Intercept'],
    ...
    def plot(self, x, ax=None, kwargs={}, offset = 0.):
        ...
        Full matplotlib kword arguments may be passed through the kwargs dictionary 
        """
        kill_label = False
