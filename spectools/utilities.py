...
def nm_to_eV(spectrum,jacobian=True):
    """
    A convenience function to transform a spectrum from a wavelength axis
    to an energy axis.
    """

    if spectrum.header[0] == 'nm':
...
def eV_to_nm(spectrum,jacobian=True):
    """
    A convenience function to transform a spectrum from an energy axis to a
    wavelength axis
    """
    
    if spectrum.header[0] == 'eV':
